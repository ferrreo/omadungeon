...
func test_title_screen_is_up_inside_the_boot_budget() -> void:
	...
	prints(
		(
			...
			% [reported, wall_ms, budget.boot_budget_seconds * 1000.0]
		)
	)
	assert_int(reported).is_greater(0)
	assert_float(float(reported) / 1000.0).is_less(budget.boot_budget_seconds)
	assert_float(float(wall_ms) / 1000.0).is_less(budget.boot_budget_seconds)
